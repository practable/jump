...
import (
	...
	"github.com/practable/jump/internal/counter"
...
func TestSendManyMessages(t *testing.T) {
	...
	duration := time.Duration(iterations+5)*5*time.Millisecond + 50*time.Millisecond
...
func TestStats(t *testing.T) {
	...
	expectedDt := 20e-3 / 7
	// arbitrary precision of 10% (was 5%); assumed sufficient to catch major mistakes in calculating Dt
	if h.Stats.Dt.Mean() > expectedDt*1.10 {
		t.Errorf("Dt stats are wrong, wanted %f got %f", expectedDt, h.Stats.Dt.Mean())
	}
	if h.Stats.Dt.Mean() < expectedDt*0.90 {
		t.Errorf("Dt stats are wrong, wanted %f got %f", expectedDt, h.Stats.Dt.Mean())
	}
